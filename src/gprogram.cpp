...
GProgram GProgram::Move(
        ...
        std::tuple<std::array<float, 3>, std::array<bool, 3>> xyz,
        int f) {
    if ((motion != GModal::motion_rapid and f == 0) or (!std::get<1>(xyz)[0] and !std::get<1>(xyz)[1] and !std::get<1>(xyz)[2]))
        return *this;
...
GProgram GProgram::MoveArc(
        ...
        float i, float j,
        int f) {
    if (f == 0 or (motion != GModal::motion_arc_ccw and motion != GModal::motion_arc_cw) or (xyz[0] == 0.0 and xyz[1] == 0.0 and xyz[2] == 0.0))
        return *this;
