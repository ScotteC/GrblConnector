...
namespace grblconnector {
    ...
    boost::regex status_report_regex{"\\<.*\\>"};
    boost::regex modal_state_regex{"(\\[GC:[\\w\\s]*\\])"};
    boost::regex wcs_offset_regex{"\\[(G5[4-9]):([+\\-]?\\d+.?\\d*),([+\\-]?\\d+.?\\d*),([+\\-]?\\d+.?\\d*)\\]"};
    boost::regex msg_regex{"\\[(MSG:)(.*)\\]"};
    ...
    void GStatus::ParseLine(std::string &line) {
        if (boost::regex_match(line, status_report_regex))
            ParseStatusReport(line);

        else if (boost::regex_match(line, modal_state_regex))
            ParseModalState(line);

        else if (boost::regex_match(line, wcs_offset_regex))
            ParseWcsOffset(line);

        else if (boost::regex_match(line, msg_regex))
            ParseMessage(line);
    ...
    void GStatus::ParseModalState(std::string &line) {
        if (boost::regex_match(line, modal_state_regex)) {
    ...
    void GStatus::ParseWcsOffset(std::string &line) {
        boost::smatch match;
        if (boost::regex_search(line, match, wcs_offset_regex)) {
    ...
    void GStatus::ParseMessage(std::string &line) {
        boost::smatch match;
        if (boost::regex_search(line, match, msg_regex)) {
